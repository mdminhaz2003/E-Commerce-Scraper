...
try:
    ...
    headers = [key for key in database_data[0].keys()]
    header_text = ""
    ...
    with open(f"{database_name.split('/')[-1].split('.')[0]}.csv", "w") as file:
        file.write(header_text)
        for single_row_data in database_data:
            ...
            if number_of_line >= 2:
                ...
                for line in range(number_of_line - 1):
                    ...
                    secondary_line_data_list[variant_inventory] = single_row_data["Variant Inventory Policy"][
                        value_index]
                    secondary_line_data_list[variant_fulfil] = single_row_data["Variant Fulfillment Service"][
                        value_index]
                    secondary_line_data_list[price_index] = single_row_data["Variant Price"][value_index]
                    secondary_line_data_list[variant_requires] = single_row_data["Variant Requires Shipping"][
                        value_index]
